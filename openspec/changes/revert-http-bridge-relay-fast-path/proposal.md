...
- Restore local-main stale-anchor diagnostics with explicit unknown owner metadata on beta.3's replay/reconnect structures, and add regression coverage for client/proxy anchor provenance, replay availability, owner source/outcome, age, and same-session status without exposing raw ids.
...
- Restore the privacy-safe credentialless `GET /api/activity/state` host-polling contract, synchronize implemented query-performance and malformed-created archive-attribution requirements, and restore their focused regression coverage.
...
- `proxy-runtime-observability`: Require failure-only HTTP bridge close attribution plus shared HTTP/direct-WebSocket settlement ordering, health neutrality, partial-failure ownership, exactly-once finalization, unusable-upstream retirement before terminal publication, shutdown cleanup quiescence, the privacy-safe activity endpoint, and explicit stale-anchor diagnostic metadata.
- `responses-api-compat`: Preserve beta.3 stale-anchor replay, reconnect, masking, and sequence behavior while adding failure-only source, replay-availability, owner, age, and session diagnostics.
- `query-caching`: Synchronize the already-implemented local-main dashboard pagination and SQLite usage-history query contracts omitted from the original candidate documentation.
...
- Restores local `GET /api/activity/state` relative to beta.3; adds no existing-route behavior change, response-schema rollback, database schema, migration, dependency, configuration, or version rollback.
- After final verification, records local `main` as a history parent while preserving the verified candidate tree exactly; leaves prior refs, live services, image artifacts, and remote refs unchanged.
