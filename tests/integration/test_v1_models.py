...
async def test_backend_codex_models_uses_bootstrap_upstream_metadata(async_client):
    ...
    expected_gpt56 = {
        "gpt-5.6-sol": ("GPT-5.6-Sol", "low", 1, ["low", "medium", "high", "xhigh", "max", "ultra"], "v2"),
        "gpt-5.6-terra": (
            "GPT-5.6-Terra",
            "medium",
            2,
        ...
        "gpt-5.6-luna": ("GPT-5.6-Luna", "medium", 3, ["low", "medium", "high", "xhigh", "max"], "v1"),
    }
    for slug, (display_name, default_effort, priority, efforts, multi_agent_version) in expected_gpt56.items():
        entry = entries[slug]
        ...
        assert entry["comp_hash"] == "3000"
        assert entry["experimental_supported_tools"] == []
        assert {"edu_plus", "edu_pro", "enterprise_cbp_automation", "sci"} <= set(entry["available_in_plans"])

    assert "most capable model yet" in entries["gpt-5.6-sol"]["availability_nux"]["message"]
    assert entries["gpt-5.6-terra"]["availability_nux"] is None
    assert entries["gpt-5.6-luna"]["availability_nux"] is None
...
@pytest.mark.asyncio
async def test_dashboard_models_excludes_ultra_reasoning_effort(async_client):
    registry = get_model_registry()
    models = [
        _make_upstream_model(
            "gpt-5.6-sol",
    ...
    assert model["supportedReasoningEfforts"] == ["low", "medium", "high", "xhigh", "max"]
