...
@pytest.mark.parametrize("body_derived", [False, True], ids=["marker-only", "body-derived"])
def test_revalidate_websocket_responses_lite_after_admission_rechecks_connection_generation(
    body_derived: bool,
):
    lite_metadata_key = "ws_request_header_x_openai_internal_codex_responses_lite"
    responses_lite_state = proxy_module._WebSocketResponsesLiteState()
    responses_lite_state.accept(model="gpt-5.6-sol", response_id="resp_ws_lite_admitted")
    accepted_generation = responses_lite_state.generation
    input_value = (
        [{"type": "additional_tools", "role": "developer", "tools": []}]
        if body_derived
        else [{"role": "user", "content": [{"type": "input_text", "text": "continue"}]}]
    )
    request_state = proxy_module._WebSocketRequestState(
        request_id="req_ws_lite_admission",
        model="gpt-5.6-sol",
        service_tier=None,
        reasoning_effort=None,
        api_key_reservation=None,
        started_at=0.0,
        previous_response_id="resp_ws_lite_admitted",
        responses_lite_model="gpt-5.6-sol",
        responses_lite_body_derived=body_derived,
        responses_lite_generation=None if body_derived else accepted_generation,
    ...
    revalidated_payload = json.loads(revalidated_text)
    expected_metadata = {"keep": "yes"}
    if body_derived:
        expected_metadata[lite_metadata_key] = "true"
    assert responses_lite_state.generation == accepted_generation + 1
    assert revalidated_payload["previous_response_id"] == "resp_ws_lite_admitted"
    assert revalidated_payload["client_metadata"] == expected_metadata
    assert request_state.responses_lite_model == ("gpt-5.6-sol" if body_derived else None)
    assert request_state.responses_lite_generation is None
    assert (revalidated_text == request_text) is body_derived
...
def test_backend_responses_websocket_responses_lite_same_model_marker_after_acceptance(
    ...
    assert sent_payloads[1]["previous_response_id"] == "resp_ws_lite_seq_full"
...
def test_backend_responses_websocket_responses_lite_trust_does_not_cross_connections(
    ...
    assert sent_payloads[1]["previous_response_id"] == "resp_ws_lite_connection_a"
...
def test_backend_responses_websocket_responses_lite_rejection_clears_marker_trust(
    ...
    assert sent_payloads[1]["previous_response_id"] == "resp_ws_lite_before_rejection"
    assert sent_payloads[2]["client_metadata"] == {"keep": "after-rejection"}
    assert sent_payloads[2]["previous_response_id"] == "resp_ws_lite_before_rejection"
...
def test_backend_responses_websocket_responses_lite_local_rejection_clears_marker_trust(
    ...
    assert sent_payloads[1]["previous_response_id"] == "resp_ws_lite_before_local_rejection"
...
def test_backend_responses_websocket_responses_lite_sequence_requires_acceptance_before_marker_only_reuse(
                ...
                json.dumps(
                    {
                        ...
                        "model": "gpt-5.6",
                        "instructions": "",
                        "previous_response_id": second_events[0]["response"]["id"],
                        "client_metadata": {lite_metadata_key: "TRUE", "keep": "yes"},
                        "input": [{"role": "user", "content": [{"type": "input_text", "text": "continue"}]}],
                        "stream": True,
                    }
    ...
    assert sent_payloads[2]["previous_response_id"] == "resp_ws_lite_after_acceptance"
...
def test_backend_responses_websocket_closes_before_replaying_exposed_sequence(
    ...
    assert log_calls[0]["status"] == "error"
    assert log_calls[0]["error_code"] == "stream_incomplete"
    assert log_calls[0]["failure_detail"] == (
        "upstream_ws_close_before_terminal_event_replay_refused_sequenced_downstream_frame"
    )
...
def test_backend_responses_websocket_direct_previous_response_disconnect_before_visible_output_replays_on_owner(
    app_instance,
    monkeypatch,
):
    first_upstream = _FakeUpstreamWebSocket([_FakeUpstreamMessage("error", error="no close frame received or sent")])
    second_upstream = _FakeUpstreamWebSocket(
        [
            _FakeUpstreamMessage(
                "text",
                text=json.dumps(
                    {
                        "type": "response.created",
                        "response": {
                            "id": "resp_ws_prev_disconnect_replayed",
                            "status": "in_progress",
                        },
                    },
                    separators=(",", ":"),
    ...
    async def fake_connect_proxy_websocket(
        ...
        *,
        ...
        connect_accounts.append("acct_ws_prev_disconnect_owner")
        excluded_account_ids_seen.append(set(getattr(request_state, "excluded_account_ids", set())))
        return SimpleNamespace(id="acct_ws_prev_disconnect_owner"), upstreams.pop(0)
    ...
    assert created_event["type"] == "response.created"
    assert created_event["response"]["id"] == "resp_ws_prev_disconnect_replayed"
    assert completed_event["type"] == "response.completed"
    assert completed_event["response"]["id"] == "resp_ws_prev_disconnect_replayed"
    assert completed_event["response"]["status"] == "completed"
    assert connect_accounts == ["acct_ws_prev_disconnect_owner", "acct_ws_prev_disconnect_owner"]
    assert excluded_account_ids_seen == [set(), set()]
    assert handled_error_codes == [("acct_ws_prev_disconnect_owner", "stream_incomplete")]
    assert len(first_upstream.sent_text) == 1
    assert len(second_upstream.sent_text) == 1
    assert json.loads(first_upstream.sent_text[0]) == json.loads(second_upstream.sent_text[0])
    assert json.loads(first_upstream.sent_text[0])["previous_response_id"] == "resp_ws_prev_anchor"
...
def test_backend_responses_websocket_direct_previous_response_disconnect_after_created_is_not_replayed(
    ...
    assert log_calls[0]["status"] == "error"
    assert log_calls[0]["error_code"] == "stream_incomplete"
    assert log_calls[0]["failure_phase"] == "upstream"
    failure_detail = log_calls[0]["failure_detail"]
    assert isinstance(failure_detail, str)
    assert failure_detail.startswith("upstream_ws_close_before_terminal_event_replay_refused_")
    assert failure_detail.endswith("previous_response_after_created_no_fresh_body")
